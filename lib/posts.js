...
// Shared "what badge does this post get in the admin" logic — used by
// both the posts list and the dashboard's recently-edited widget, so the
// two can't quietly drift into showing different labels for the same
// post.
export function getPostStatusInfo(post) {
  const now = new Date();
  const scheduledFuture = post.scheduled_for && new Date(post.scheduled_for) > now;
  const isLive =
    post.status === "published" || (post.status === "scheduled" && post.scheduled_for && new Date(post.scheduled_for) <= now);
  if (isLive) return { label: "Published", color: "bg-river/[0.1] text-river" };
  if (scheduledFuture) {
    return {
      label: `Scheduled — ${new Date(post.scheduled_for).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}`,
      color: "bg-mustard/25 text-ink",
    };
  }
  return { label: "Draft", color: "bg-steel/[0.12] text-steel" };
}
...
// The single newest published (or now-live scheduled) post — used by the
// "always jump to whatever's newest" redirect at app/latest-article, since
// there's no fixed URL that could ever be "the latest one" the way
// /latest (the full listing) or /article/[a fixed slug] are. A dedicated
// `.limit(1)` query rather than reusing getPublishedPosts()[0]: that
// fetches every published post's full row just to throw away all but
// one, which is fine for a page that was already listing them all
// anyway, but wasteful for a route whose only job is "redirect somewhere".
export async function getLatestPublishedPost(supabase) {
  const { data, error } = await supabase
    .from("posts")
    .select("slug")
    .or(VISIBLE_FILTER)
