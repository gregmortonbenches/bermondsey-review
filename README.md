...
- **`components/HeaderArches.jsx` — an illustrated strip above the wordmark, walking back part of the "illustration-free masthead" call above.** A repeating line of brick viaduct arches — the actual railway out of London Bridge through Bermondsey, a real local landmark rather than a generic skyline — with a train that crosses it every so often. Took three tries to land on the right *style*, not just the right subject: the first two passes were built from filled shapes (a plain cutout circle, then an ellipse with an outline and voussoir fan-lines added on top of a fill) and read as illustration-by-committee rather than a single confident drawing, against reference photos of the real Bermondsey arches. The version here is pure line art instead — one stroke weight throughout (`STROKE = "1.75"`), no fills anywhere, flat *segmental* arches (an elliptical arc, `ARCH_RY` well under half of `ARCH_RX`) rather than tall semicircles, matching a real viaduct's proportions. The parapet — where the line itself runs, on top of the brick — is one plain `<line>` floating clear above the arch crowns with a visible gap, not connected to them by verticals; the arches tile via a single SVG `<pattern>` of just that one stroked path repeated (no `viewBox` on the outer `<svg>`, so pattern units are real pixels — the strip repeats by count as it gets wider rather than stretching non-uniformly on wide screens). The train is a second small SVG, also plain stroke with no fills, sized and positioned so its skirt sits exactly on the parapet line's y-coordinate, same as the arches relate to it — redrawn once more against real photos of the actual Southeastern Class 707s that run this line: a streamlined nose and a flush skirt with no visible wheels, not the steam-engine-with-funnel-and-smoke first sketched (this line hasn't run steam since long before anyone reading it was born). Nose faces right, the leading edge for a sprite whose `@keyframes` move it left-to-right. Three carriages, not one — the body is a single continuous outline (real EMUs are articulated, not separate boxes coupled end to end), with two plain vertical joint lines marking where one carriage ends and the next begins. Crosses via one `@keyframes` loop (`.header-train`, `app/globals.css`) — mostly holds off-screen to the left, then takes about 10s to cross, once roughly every 42s, so it reads as an occasional flourish rather than a distracting loop. No client JS at all; `prefers-reduced-motion` was already handled globally in `globals.css` (every animation site-wide, not just this one), so it needed no accessibility handling of its own. Kept as a separate band above `MastheadNav` rather than folded into the wordmark row, so the wordmark itself stays exactly as restrained as the entry above describes — the page as a whole isn't fully illustration-free any more, but the one place that mattered most (competing with the title) still is.
