...
## Admin layout canvas, phase 3: Submissions becomes real, editable content

- **The pragmatic option, not the purist one.** Two ways to close this were on the table: move Submissions onto the full `pages`/`/admin/pages` CMS (rich blocks, but needs a page row created by hand in the live admin — not something this session can do without write access to the real Supabase), or extend `page_copy` — already admin-editable, already wired into `/admin/layout` — to cover the actual substance of this one page, not just its heading. Went with the second: it ships now, entirely within this session, and gets the *content that matters* (the four guideline answers, the submissions address) into admin, not just the title.
- **`page_copy.submissions` gained four named fields** (`whatWereLookingFor`, `whoFrom`, `howToSend`, `whatHappensNext`) **plus `email`** — deliberately not one free-text blob for the whole page. The four questions themselves stay fixed (not admin-editable); only the answers are. Collapsing to one big field would have been simpler to build but would've thrown away the one thing that makes this page scannable as guidance rather than a wall of text — an admin should be able to reword an answer without being able to accidentally flatten the structure.
- **`components/SubmissionsBody.jsx`** is the same page, extracted — `app/submissions/page.jsx` is now a thin wrapper (metadata + `PageViewTracker`) around it, same split `ArchiveBody.jsx` already uses, so the real route and the new admin preview frame render one component, not two that can drift apart.
- **A genuine "Submissions" tab in `/admin/layout`**, alongside Home/Articles/Guess the Spot — `PageCopyEditCanvas` handles the title/description exactly like Archive and Geoguesser already do, with the four answer fields and the email address passed in as its `children` slot (the same slot those two use for read-only context, except these are wired to real `onChange`/autosave, not just shown for reference). A matching `submissions-frame` preview route was added too, so the "Preview ↗" link doesn't silently fall back to Home the way it would have without it.
- **Closes the actual navigation gap phase 2 left open**: `/submissions` is now in `resolveCanvasNav()`'s map, so the footer's own Submissions link — previously an inert click inside any canvas — now switches straight to this tab.
- **Verified with real interaction, not just a read**: clicked through to the tab, confirmed title/description/all four fields/email all populated with their real values, edited one field and confirmed its siblings held their own values (the merge logic), and confirmed the public `/submissions` route still renders byte-for-byte the same default copy as before this change, now sourced from data instead of hardcoded JSX.

## Admin layout canvas, phase 2: nav clicks actually go somewhere, not just article cards

- **Extends phase 1's pattern to the two things it didn't cover yet.** Phase 1 made article/cartoon cards link to their real editor instead of being swallowed by the canvas's link-suppression; this round does the same for `PuzzlesSection`'s Crossword/Geoguesser cards (`adminEditable` now points them at `/admin/crossword`/`/admin/geoguesser` — the real management screens those games already have, since there's no per-card "editor" the way a post has one) and, separately, for the masthead's own nav links, which used to just do nothing at all when clicked inside any canvas.
- **New `resolveCanvasNav()` in `canvasNav.js`** maps a clicked link's href to what should actually happen: `/latest`/`/latest-article` → switch to the Articles tab, `/geoguesser` → the Guess the Spot tab, `/#puzzles`/`/#cartoons` (the masthead's actual nav_links shape) → scroll to that section on Home. Anything it doesn't recognise (`/submissions`, `/crossword`, a truly external link) still resolves to nothing and falls through to the existing plain suppression — this only adds destinations, it doesn't change the safe default for everything else.
- **The two canvases in `AdminLayoutTabs.jsx` are now wrapped in one shared click handler** that tries `resolveCanvasNav()` first, before each canvas's own `suppressCanvasNavigation` gets a turn — so a recognised link changes tabs or scrolls, and an unrecognised one is suppressed exactly as before. Clicking a same-page anchor from a *different* tab (e.g. "Puzzles" while looking at Articles) switches to Home and then scrolls once Home has actually mounted, rather than trying to scroll to an element that doesn't exist yet in the currently-rendered tab.
- **Verified with real clicks, not just a DOM read** — rendered `AdminLayoutTabs` directly in a harness (bypassing the real `/admin/layout` route, which needs an authenticated session this environment doesn't have) with mock nav links standing in for the masthead's real ones, and drove it with Playwright: clicking "Reviews" landed on the Articles tab, "Guess the Spot" on its tab, and clicking "Puzzles" from a different tab switched to Home and put the `#puzzles` section in view — all without the page ever actually navigating away. Confirmed the article card's own admin-edit link is untouched by the new handler.
- **Still ahead:** anything not yet backed by real content (Submissions being the standing example) has nowhere for this navigation to land — that's phase 3, a content-model change, not a click-handling one.

## Admin layout canvas, phase 1: article/cartoon cards now actually link to their editor

- **First concrete step of the "make /admin/layout reach anything, Squarespace-style" scoping** discussed and agreed on before this: rather than a big-bang rewrite, prove the model on the three destinations that already exist. Clicking an article or cartoon card inside `/admin/layout`'s Home canvas used to do *nothing* — the canvas's own link-suppression (`canvasNav.js`, there so clicking a nav link doesn't navigate the admin away entirely) swallowed the click with no escape hatch, since these cards only ever carried a plain public `/article/[slug]` link.
- **`ArticleCard.jsx`, `ArticleGrid.jsx`, and `CartoonsSection.jsx` all take a new `adminEditable` prop** (default `false`, so the public site's own rendering of all three is untouched). When true, each card's link points at `/admin/posts/${article.id}/edit` instead of the public article, and carries `data-canvas-allow="true"` — the exact escape hatch `canvasNav.js` already defines for "a link to a different admin screen," used elsewhere for the geoguesser round-management link. No new suppression mechanism, just finally using the one that was already there.
- **Wired at the three places these components render inside the admin**: the Home canvas's live `ArticleGrid`/`CartoonsSection` (`LayoutCanvas.jsx`), and the featured card + archive list built server-side in `app/admin/(dashboard)/layout/page.jsx`. `PuzzlesSection` deliberately untouched — its links go to the actual game pages, which have their own dedicated management screens already reachable via the existing `data-canvas-allow` link next to the round preview; there's no per-card "edit this" destination for a game the way there is for a post.
- **Verified structurally, not by clicking a real `/admin/layout`** — that route requires a real authenticated Supabase session this environment doesn't have. Rendered all four components with and without `adminEditable` in a harness and confirmed the exact resulting `href`/`data-canvas-allow` attributes: public renders completely unchanged, admin renders pointing at the right post's real edit screen.
- **Phases 2 and 3 (generalizing beyond these three destinations, migrating hardcoded pages like Submissions onto real content so there's something to navigate to) are still ahead** — this only proves the pattern works on what already exists.
...
- **`/rss.xml`** (`app/rss.xml/route.js`) — a real Route Handler, not a special-file convention like `sitemap.js`/`robots.js`: RSS has no built-in Next.js file. Reuses `getPublishedPosts`, filtered to article/cartoon (same restriction `sitemap.js` already applies, since those are the only types with a live route), capped at the most recent 50. Hand-escapes XML entities in title/description/author — the same text that's safe to drop straight into JSX isn't safe to drop straight into XML, and one `&` in a headline would otherwise break the feed for every subscriber, not just that post. Discoverable via a `<link rel="alternate" type="application/rss+xml">` added to the root layout's metadata, the same way `robots.js` already points crawlers at the sitemap. Verified the output is well-formed XML with an actual parser, not just eyeballed.
- **A print stylesheet** (`@media print` in `globals.css`, plus a `.no-print` utility) — genuinely on-brief for a site whose whole design language is "printed-paper-first," and there was, until now, no print handling at all: hitting print on an article got the full masthead, nav, and the reader's own text-size/share controls along with it. `.no-print` now sits on `Masthead.jsx`'s `<header>`, `Footer.jsx`'s nav and social icons, and `ArticleSidebar.jsx`'s "Text size" and "Share" blocks — everything that only means something on a screen. Headline, byline, date, category and body text print exactly as they read. Verified with Playwright's print-media emulation, not just the CSS in isolation.
- **`Article` JSON-LD** on `/article/[slug]` — separate from the OpenGraph tags `generateMetadata` already sets (those control how a shared link *previews*; this controls whether Google can parse the page as an article at all, which OG tags don't do). Built from data already on the fetched `post`, so no new query. Checked the generated object against real and sparse post data directly — valid JSON either way, and every optional field cleanly drops out instead of shipping as `null` when the source data's missing.
- **A Submissions page** (`app/submissions/page.jsx`), with a footer link (`Footer.jsx`, next to the copyright line). Static guidelines copy in the site's own voice — what's accepted, who from, how to send it, what happens next — plus a single river-accent mailto CTA, the one place on the page colour is spent. Title/dek pulled through the existing `page_copy` admin-editable pattern (`lib/theme.js`), same as Archive and Geoguesser already do, so an editor can update the heading without a code change even though the body copy itself is hardcoded. The submissions address (`submissions@bermondseyreview.co.uk`) is a placeholder — swap it for the Review's real inbox.
...
- **New `lib/categories.js`** exports `CATEGORY_INK`, a fixed hex per category (Books keeps the site's own river blue; Bermondsey, Film, Culture and Cartoon each get their own — an ochre, a verdigris teal, a plum, a forest green), and `categoryInk()`, a lookup with an ink fallback for anything unrecognised. Five categories, matching `CoverArt.jsx`'s five placeholder icons one for one. Not admin-editable — same footing as the icons themselves, hardcoded rather than added to `site_settings`.
- **This is a printer's-spot-colour system, not a new palette:** every existing category kicker (`ArticleCard.jsx` both sizes, `PostRenderer.jsx`'s article hero and cartoon branch) swapped its hardcoded `text-river` for `categoryInk(category)` — colour these already had, now actually meaning something instead of being river by coincidence. `ArticleGrid.jsx`'s tiles get a plain 3px colour rule above the headline, no text — deliberately not reintroducing the category label that was removed on request; the rule alone restores sort-by-colour-at-a-glance at a fraction of the visual weight a caption would cost.
- **`PuzzlesSection.jsx`'s cards dropped their hairline border** for the same flat `bg-steel/[0.05] hover:bg-steel/[0.09]` fill every other card and tile on the site now uses — the border was a deliberate choice when it shipped (see the file's own comment, kept and extended rather than deleted), made before that flat-fill idiom existed anywhere else; it had quietly become the one card format on the page still drawing a line around itself.
- **Verified together** — featured card, a four-category grid, Puzzles, an article page, and a cartoon page — confirming each kicker/rule takes its own category's colour rather than all still matching by coincidence, and that Puzzles reads as quiet rather than "oversized," the exact failure mode its own comment already warned against.
...
- **First shipped at 5px — the middle of the original "4-6px greater than the font size" ask — then moved to 8px.** 5px was technically inside the requested range but read tighter than "airy and open" on an actual screen, closer to a normal-density paragraph than the more open one being asked for; landing there was playing it safe within the number rather than aiming at the actual result. 8px is where it visibly opens up without the lines drifting apart from each other — closer to where real magazine body settings tend to sit. Verified directly at both values: the computed `font-size`/`line-height` gap comes out to exactly the target at all three text-size presets (small/medium/large) and both breakpoints — six combinations each time, no drift at any size.
- **The admin block editor's own live canvas got a matching but simpler change** (`EditableParagraph` in `BlockEditor.jsx`): its paragraph text is a fixed `text-lg` (18px), not tied to the reader's adjustable size at all, so there's no variable to reference — it's just `leading-[26px]`, the same "18px + 8px" target spelled out directly, keeping the "what you edit is what ships" canvas actually matching the public render.
...
- **First fix attempt (`scroll-px-4`) corrected the scroll position but not the underlying problem.** CSS padding never clips overflowing content — it only positions the laid-out track relative to the box — so the next column's grid track (which legitimately overflows past the content box; that's what makes the rail scrollable at all) kept rendering a sliver into the reserved padding-right space, and only the element's own *border-box* edge is what `overflow-x-auto` actually clips, not its padding edge. Confirmed directly by measuring `scrollLeft` (16, should be 0) before the fix and the rail's own rendered box before and after.
- **Actual fix: dropped the mobile bleed entirely.** No more `-mx-4 px-4`/`scroll-px-4` — the rail's tiles now sit inset the same amount as everything else on the page, including the section header directly above them, rather than reaching the literal screen edge. With no padding left to protect, there's no space left for an overflowing track to show through, so the cut is exact regardless of gap width. This is a real trade — the tiles lose their edge-to-edge feel on mobile — but it's the version that's actually clean rather than the version that mostly looks clean.
- Verified by reading the rail's own computed box directly (`getBoundingClientRect`, `scrollLeft`, computed padding) rather than trusting a screenshot alone — a first round of screenshots looked fixed but turned out to be stale from dev-server HMR caching, caught by cross-checking the live DOM's actual class list against what should have shipped.

## Reviews section is a rail again, column-major fill; masthead viaduct now true edge-to-edge

- **The homepage's "more articles" section scrolls again instead of wrapping into new rows.** It was a wrapping `grid sm:grid-cols-2` capped at four tiles — every extra article cost another row of page height, so it had to stop somewhere. Now it's a horizontally-scrolling rail (`grid-auto-flow: column`, native overflow + scroll-snap, same mechanism the original `ArticleCarousel` used) with no cap: one row on desktop with exactly two tiles in view, two rows on mobile with the next column peeking at the edge as a scroll affordance.
- **Fill order is column-major, not row-major:** the first article is top-left, the second sits under it, the third starts the next column, the fourth sits under *that* — "top, bottom, top, bottom," not "left, right, left, right." That's `grid-auto-flow: column` with a fixed row count (`grid-rows-2` on mobile, `grid-rows-1` on desktop) doing the actual work, not a hand-rolled chunking function — the DOM stays in plain array order and CSS decides the visual position.
- Column width (`auto-cols`) is what controls how many tiles are actually in view: `72%` on mobile so the next column visibly peeks in, and `calc(50% - 0.375rem)` on desktop — exactly half the rail minus one gap — so two sit fully in view and a third is cut cleanly at the edge rather than half-showing an arbitrary sliver.
- **The masthead's viaduct — the repeating brick-arch pattern behind the wordmark — now runs to the literal edge of the screen**, not just to the site's usual `px-4`/`px-6`/`px-12` content padding. That padding used to live on the wrapper `<div>` around the whole masthead in `Masthead.jsx`, which constrained the arch background the same as the text above it. It now lives on an inner `<div>` inside each of `MastheadNav.jsx`'s two rows, wrapping only the actual text content (wordmark, nav links, Subscribe) — `HeaderArchesBackground` is a sibling of that inner div, absolutely positioned and sized against the *outer*, now-unpadded row, so it reaches the true viewport edge on both sides while the text keeps exactly the same inset it always had. Confirmed by measuring the arch SVG's own bounding box at both breakpoints: `x: 0`, width equal to the full viewport width, at 390px and 1280px alike.
...
- **Portrait rectangles on mobile (`3/4`), squares on desktop.** A tile taller than it is wide reads as a page rather than a panel, which is the right instinct for a paper, and it gives the artwork real vertical room — which is what matters once the real per-article illustrations land. It costs scroll length deliberately: four portrait tiles are a long section, but each is worth stopping at. Desktop keeps the square, where two sit side by side at half a column each and portrait would run absurdly tall. (Built this landscape first, which was a misreading — landscape is more compact but reads as a panel, and squeezes the artwork exactly where it can least afford it.)
...
- **`river` is the one accent** — category labels, the drop cap, a quote's rule, body buttons, the Subscribe button, focus rings, the newsletter drawer, selected states. **`brick` is now purely functional** — errors, alerts, required-field markers, a wrong crossword letter — and changed from bright yellow (`#F5C518`) to a brick red (`#A63D2F`), which the name always suggested and which actually reads as "something is wrong". No yellow anywhere now.
- **Tints removed from every editorial surface.** The article hero band and the homepage featured card are plain paper with a hairline instead of a category wash; `CoverArt`'s placeholder illustrations are ink on a faint grey rather than coloured per category, which mattered more than anything else here — a placeholder appears wherever a piece has no photo, so on a listing page most of the artwork was coloured by default.
- **Other colour that had to move with it:** the Subscribe button dropped its fixed pale `#87D6FF` for the accent at full strength (with white text, which the pale blue couldn't carry); the Guess the Spot map's "correct" pin was the old yellow, and is now ink-for-your-guess against accent-for-the-real-spot, two values rather than two hues so it survives a colour-blind reader; the crossword's selected cell was a literal yellow (the NYT-ish convention) and is now the accent at two strengths, keeping selected-cell vs active-word distinct.
- **`categoryFamily` in `lib/articles.js` is no longer called by anything.** Left in place, clearly marked, with the reasoning above recorded on it — it's safe to delete, and the note is there so deleting it doesn't lose why.
- Not touched: the block editor's own optional per-block "River tint"/"Brick tint" backgrounds (an opt-in choice an admin makes per block, not the default look), and admin UI colour generally, which is tooling rather than the paper.
- **This changes code defaults only.** `brick`/`river` are admin-editable and stored in `site_settings`, so a site with values already saved keeps them until someone updates them in `/admin/theme`.
...
- **Each tile is one continuous category-tinted square**, headline at the top, illustration floating large and low beneath it — not the split "text band above, image band below" a card usually gets here. That structure depends on the artwork being a line illustration on a transparent ground, so it reads as floating *in* the tile's colour; a rectangular photo would need its own hard edge and put the seam back. The article's cover photo isn't used on the homepage at all now — it stays at the top of the piece itself. No category label on the tile either: the category still picks the tint and the illustration, it just isn't spelled out.
- **Deliberately tighter spacing than the rest of the site** — a 8/12px gap between tiles rather than the usual 24/32px, so the grid runs nearly edge-to-edge the way Apple's does. Everything else stays in this paper's own language rather than Apple's: square corners, flat tints, hairline borders, no gradients, shadows, or pill buttons.
- **Capped at four tiles**, unlike the rail it replaced. A scroll rail cost the same vertical space no matter how many articles it held; a grid of large squares does not, and rendering every past article as its own square would have run the homepage to many screens. The full archive is one click away behind the nav's Reviews link (`/latest`) — worth knowing this is now the only route to older pieces from the homepage.
- **The stored section type stays `"carousel"`.** Saved `page_layouts` rows already use that string, and renaming it would orphan every existing layout, so only the rendering and the admin-facing label ("More Articles") changed. The section's DOM id stays `carousel` to match. Its two "articles visible on mobile/desktop" settings were removed from the layout builder: those were per-view counts for a scroll rail, which a fixed 2-up/1-up grid has no equivalent for, and leaving them would have meant two controls that silently did nothing. A saved layout may still carry their old values; nothing reads them now.
- Verified the section renders with its header defaults intact, that the four-tile cap actually holds when handed seven articles (the extras never reach the DOM), and that the admin layout canvas still compiles and loads after the swap.

## Fixed: pressing Enter in a text block didn't actually start a new paragraph

- **A real, severe bug, not a missing nicety**: typing a second paragraph in a text block and pressing plain Enter (not Shift+Enter) produced no visible break at all once published — the two paragraphs shipped glued into one run-on line with zero space between them. Confirmed by capturing the actual browser output and running it through the real sanitizer: Chrome's native Enter behaviour wraps the new line in a bare `<div>`, which was never in `sanitizeBlockHtml`'s allowlist (`components/BlockContent.jsx`) — sanitize-html's default response to a disallowed tag is to unwrap it (drop the tag, keep the text), so the `<div>` vanished and took the only marker of where one paragraph ended and the next began with it. Shift+Enter was fine the whole time — its plain `<br>` was already allowed — which is presumably why this went unnoticed for as long as it did.
- **Fixed at the source**: `EditableParagraph` (`components/admin/BlockEditor.jsx`) now intercepts a plain Enter itself, inserting a `<br class="pb">` marker instead of letting the browser's own div-wrapping happen at all. `sanitizeBlockHtml` now explicitly allows that one class on `<br>` (`allowedClasses: { br: ["pb"] }` — not classes generally, just that literal value), and both the editor's live canvas and the public renderer give `br.pb` its own `[&_br.pb]:block [&_br.pb]:mt-3` rule, rendering as a proper paragraph-sized gap rather than a plain line break. Shift+Enter is untouched — still a plain, unclassed `<br>`, still just a soft line break within one paragraph.
- **Getting the marker's insertion point right took two tries.** The first attempt used `execCommand("insertHTML", ...)`, matching this file's existing bold/italic/link commands — but it reliably left the caret positioned *before* the newly inserted node rather than after it (confirmed by dumping `innerHTML` after every keystroke in isolation), so continuing to type pushed each new paragraph's text in front of the marker instead of behind it, silently reordering everything typed after the first Enter. Switched to inserting the `<br>` directly via the Selection/Range API and explicitly moving the range — which still weirdly snapped back to "before" when the marker sat at the very end of the content with nothing following it, apparently a genuinely ambiguous caret position for contentEditable's own editing model, not something fixable by just asserting a different Range boundary. What actually worked: append an invisible zero-width-space text node right after the `<br>` and park the caret inside *that*, which gives the browser's caret logic an unambiguous anchor. The zero-width space itself never reaches storage — it's stripped from whatever this component passes to `onChange`, so it only ever exists transiently in the live DOM while typing.
- **A related rough edge, caught during verification, not the original bug**: pressing Enter on a genuinely empty text block inserted a floating marker with nothing on either side of it, and also hid the "Write a paragraph…" placeholder (which only recognised a bare `<br>` as "empty," not `<br class="pb">`). Fixed by no-op'ing the handler when the block has no text yet — there's nothing yet for a break to separate.
- Verified end-to-end with the real `BlockEditor` and the real public `BlockContent` renderer mounted side by side in the same harness (not a mock of either): typed three paragraphs with a Shift+Enter line break embedded in the middle one, confirmed the stored block JSON carried exactly the right markers in exactly the right places with no stray zero-width characters, and confirmed the editor's own live canvas and the public render produced pixel-identical spacing — a real paragraph gap around the Enter presses, no gap at all around the Shift+Enter.

## New block type: the Bermondseyometer, a postcode-in-percentage-out widget

- **A postcode search box for the "boundaries of Bermondsey" article**, letting a reader type their own postcode and see what share of the 39 survey respondents drew that spot inside their own hand-drawn Bermondsey — the same underlying data as the ranked pub-list block, from a different angle. Ported from a standalone prototype (built outside this codebase, shared as a single HTML file) rather than built from scratch: the boundary-drawing survey's 39 GeoJSON polygons and the point-in-polygon math moved over essentially unchanged, since that's genuinely reusable logic with no UI opinions baked in.
- **Deliberately not a port of the prototype's own UI.** That had a meter bar and "compare to Bermondsey Street / Surrey Quays" cards on top of the percentage — engaging as a standalone toy, but more than an inline article widget needs. What shipped is just the essentials: a postcode, the percentage, and a one-line classification/verdict (kept from the prototype, since a bare number reads as a statistic where the classification reads as a verdict) — no meter, no comparison cards.
- **A fixed, non-configurable block**, unlike the ranked-list or HTML blocks — the 39 boundary polygons live in `lib/bermondseyometer.js`, not in the block's own stored JSON, since nobody's re-entering 39 hand-drawn shapes through an admin form. Inserting the "Postcode checker" block from the inserter menu is the only thing an admin does with it; the admin canvas shows the exact same live, working widget a reader gets (same "true visual canvas" convention every other block follows), not a config form.
- **Looks up postcodes via postcodes.io**, a free, keyless public API — the postcode a visitor types goes straight from their own browser to postcodes.io, never through this site's server, so there's nothing of theirs to store even if this app wanted to.
- Verified with the real postcodes.io API directly (`curl`) to confirm the endpoint and response shape, then via Playwright with the network response mocked to the same real shape (more deterministic than depending on a live third-party API for a repeatable test) — checked a real Bermondsey postcode's percentage and classification against the same scoring function run standalone in Node (92%, "Proper Bermondsey," matching the prototype's own reference figure for the same spot), a postcode outside every drawn boundary (0%, "Absolutely not, apparently" — no NaN or off-by-one at the floor), a not-found postcode (the 404 error path), and an empty submit.
...
- **What actually got removed:** `HeaderWorm` (the SVG worm itself, `components/HeaderArches.jsx`) and its two render sites in `MastheadNav.jsx` (mobile and desktop wordmark rows); `components/HeaderWormSpeed.jsx` entirely — the "use client" component that measured `.header-worm`'s rendered width and wrote `--worm-cycle-duration`/`--worm-cross-delay` onto the document root so the crossing kept a constant real-world speed regardless of viewport width — along with its own render site and explanatory comment in `MastheadNav.jsx`; and the `.header-worm` rule plus `@keyframes header-worm-cross` in `globals.css`, including the long comment above them describing the speed derivation.
- **What deliberately stayed:** `HeaderArchesBackground` — the brick viaduct arch pattern the worm used to cross in front of — and every constant it shares with the old `HeaderWorm` function (`STROKE`, `BAND_H`, `TILE_W`, `PIER_X`, `ARCH_W`, etc.), since those position the arch band itself and have nothing to do with the worm specifically. The masthead's arch illustration is unchanged in both rows; only the creature that used to cross in front of it is gone. `ArticleSidebar.jsx`'s own doc comment, which had cited `HeaderWormSpeed.jsx` as prior art for writing CSS custom properties onto `document.documentElement` from a client component, was reworded to describe the technique on its own rather than point at a file that no longer exists.
- Verified via Playwright at mobile (390px) and desktop (1280px): zero `.header-worm` elements in the DOM at either width, both arch backgrounds still rendering (mobile row + desktop row, one visible at a time per breakpoint as before), no console errors from the removed imports. A fresh `grep -rn "worm\|Worm"` across the codebase turns up nothing left but this README entry and a couple of plain-English "used to" mentions in code comments — no live references.
...
## Article body text: a slightly larger standard size on mobile

- **Mobile's own default article paragraph size nudged up** (18px → 19px), desktop's left exactly as it was — a visitor never touches the sidebar's text-size control at all by default, so this is really about mobile's *baseline* being a touch more comfortable to read, not the text-size feature itself. The `small`/`large` presets moved with it ("adjust options for changed font size accordingly," as asked) — mobile's whole three-step ladder shifted up by that same one-step difference (17px/19px/22px) rather than leaving small/large at their old values while only medium changed, so picking "small" or "large" on mobile still reads as one deliberate step away from mobile's own new baseline. Desktop's ladder (16px/18px/21.6px) is untouched.
- **Two CSS custom properties now, not one** (`--article-font-size-mobile` / `--article-font-size-desktop`) — `BlockContent.jsx`'s paragraph text reads the mobile one at the base breakpoint and switches to the desktop one at `sm:` (640px+, the same breakpoint the rest of the site already treats as the mobile/desktop line), via a genuine CSS media query rather than any JS viewport-detection. `ArticleSidebar.jsx`'s text-size buttons write *both* properties together, unconditionally, on every choice — the CSS media query is what actually decides which one applies at any given moment, so resizing the browser window just changes which property was already being read; the sidebar itself never needs to know or react to the current viewport width. Both properties' fallback values (used wherever no `ArticleSidebar` is mounted — any admin block-editor preview, any page type that doesn't render one) are "medium" at their own width, so those places also get mobile's own slightly-larger default now, not desktop's value stretched across every screen size.
- Verified via Playwright at both a 390px and a 1280px viewport: confirmed mobile's untouched default measures 19px and desktop's measures 18px (unchanged), and confirmed clicking "small"/"large" on each width applies that width's own ladder (17px/22px on mobile, 16px/21.6px on desktop, matching the pre-existing desktop values exactly).
