...
- **`components/HeaderArches.jsx` — an illustrated strip above the wordmark, walking back part of the "illustration-free masthead" call above.** A repeating line of brick arches — the actual Victorian viaduct that carries the railway out of London Bridge through Bermondsey, a real local landmark rather than a generic skyline — with a train that crosses it every so often. The arches tile via a single SVG `<pattern>` (no `viewBox` on the outer `<svg>`, so pattern units are real pixels: the strip repeats by count as it gets wider instead of stretching non-uniformly on wide screens). First pass drew the arch opening as a plain filled circle cut from a solid brick rect — no linework at all, so it read as a row of flat scalloped bunting, not architecture. Redone to match the site's actual illustration language (the crossword grid/Tower Bridge line art in `PuzzlesSection.jsx`): an ellipse rather than a circle (taller than it is wide, closer to a real arch's proportions than a shallow wide scallop), a thin ink-coloured stroke tracing its edge for definition, and a fan of five thin "voussoir" lines radiating from the arch's centre to its own curve — standing in for the coursed brick a real arch is built from, the one detail that reads as brick rather than "a hole in a rectangle." (Still built via the same fill → stroke → redraw-the-deck-solid-on-top trick as before, so the ellipse's upper half — which pokes above the springline by construction — gets clipped back to one crisp flat roofline rather than showing through.) The train got the same treatment: a stroked line-art locomotive (body, cab window, funnel, a couple of smoke puffs, wheels with a coupling rod) in place of the first pass's solid-fill blob that didn't read as anything at its deployed size. Crosses via one `@keyframes` loop (`.header-train`, `app/globals.css`) — mostly holds off-screen to the left, then takes about 10s to cross, once roughly every 42s, so it reads as an occasional flourish rather than a distracting loop. No client JS at all; `prefers-reduced-motion` was already handled globally in `globals.css` (every animation site-wide, not just this one), so it needed no accessibility handling of its own. Kept as a separate band above `MastheadNav` rather than folded into the wordmark row, so the wordmark itself stays exactly as restrained as the entry above describes — the page as a whole isn't fully illustration-free any more, but the one place that mattered most (competing with the title) still is.
