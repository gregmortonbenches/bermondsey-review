...
- **`components/HeaderArches.jsx` — an illustrated strip above the wordmark, walking back part of the "illustration-free masthead" call above.** A repeating line of brick arches — the actual Victorian viaduct that carries the railway out of London Bridge through Bermondsey, a real local landmark rather than a generic skyline — with a train that crosses it every so often. Built as a single SVG `<pattern>` tile (a brick rect, a big circular void punched out for the arch opening, then the deck redrawn solid on top of that so the void's round top doesn't show through — gives a crisp flat roofline with piers as the untouched brick either side of each arch) rather than a data-URI background-image, and deliberately has no `viewBox` on the outer `<svg>` so the pattern's units are real pixels: it repeats by count as the strip gets wider instead of stretching non-uniformly on wide screens. The train is a second, small SVG (a simple flat locomotive silhouette matching the site's existing icon style — `PuzzlesSection.jsx`'s crossword grid/Tower Bridge line art) that crosses via one `@keyframes` loop (`.header-train`, `app/globals.css`) — mostly holds off-screen to the left, then takes about 10s to cross, once roughly every 42s, so it reads as an occasional flourish rather than a distracting loop. No client JS at all; `prefers-reduced-motion` was already handled globally in `globals.css` (every animation site-wide, not just this one), so it needed no accessibility handling of its own. Kept as a separate band above `MastheadNav` rather than folded into the wordmark row, so the wordmark itself stays exactly as restrained as the entry above describes — the page as a whole isn't fully illustration-free any more, but the one place that mattered most (competing with the title) still is.
