  ...
  return (
    <>
      ...
      {/* Mobile: just the title now — no Subscribe button, no hamburger —
          ...
          The horizontal padding that used to live on Masthead.jsx's own
          wrapper (px-4 sm:px-6 lg:px-12) now lives on the *inner* div
          here instead, not on this outer row. HeaderArchesBackground is
          an absolutely-positioned sibling of that inner div, sized via
          `inset-x-0`/`w-full` against this outer row — so keeping the
          outer row itself unpadded is what lets the arch pattern tile
          all the way to the actual edge of the screen, while the padded
          inner div still keeps the title text away from it. Same split
          on the desktop row below.
          Extra bottom padding on the inner div (pb-12, vs. a plain py-4)
          beyond what the title itself needs: HeaderArchesBackground's
          arch band is bottom-anchored (`bottom: 0`) so its ground level
          always abuts this row's own hairline, and the row needs enough
          room below the title for that fixed-height band to show
          without `overflow-hidden` clipping it down to a sliver. pb-12
          gives an 8px gap between the title and the parapet above the
          arches (pb-12's 48px, minus the band's own 40px height).
      ...
      <div className="relative isolate overflow-hidden flex sm:hidden hairline-b">
        <HeaderArchesBackground id="mobile" />
        <div className="px-4 sm:px-6 lg:px-12 flex items-center justify-center pt-4 pb-12 gap-2 w-full">
      ...
      {/* Desktop: wordmark dead-centre, Subscribe pinned right — a true
          ...
          Same inner-div padding split as the mobile row above, and the
          same extra-bottom-padding reasoning — pb-14's 56px leaves a
          16px gap above the arch band. */}
      <div className="relative isolate overflow-hidden hidden sm:block hairline-b">
        <HeaderArchesBackground id="desktop" />
        <div className="px-4 sm:px-6 lg:px-12 grid grid-cols-[1fr_auto_1fr] items-center pt-6 pb-14 gap-4">
      ...
      {/* Nav — a centred row, serif to match the wordmark rather than
          ...
          rather than a deliberate double border. Its own horizontal
          padding now, same as the two rows above — it lost the padded
          wrapper Masthead.jsx used to provide, along with them. */}
      <nav className="px-4 sm:px-6 lg:px-12 flex flex-wrap items-center justify-center gap-x-6 sm:gap-x-8 gap-y-2 py-4 font-display text-sm text-ink">
        {links.map((link) => (
